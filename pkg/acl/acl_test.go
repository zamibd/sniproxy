...
func TestMakeDecision(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			connInfo: mockConnInfo("1.1.1.1", "api.cde.capp.bka.sh"),
		...
		},
		{
			...
			connInfo: mockConnInfo("77.77.1.1", "eventapi.capp.bka.sh"),
			config:   configs["acl_cidr.yaml"],
			expected: Accept,
		},
		{
		...
		},
		{
			...
			connInfo: mockConnInfo("1.1.1.1", "images.capp.bka.sh"),
			config:   configs["acl_domain_cidr.yaml"],
		...
		{
			// domain in domains.csv, ip ACCEPT in cidr.csv
			connInfo: mockConnInfo("77.77.1.1", "cmsimages.capp.bka.sh"),
			config:   configs["acl_cidr_domain.yaml"],
			expected: ProxyIP,
		},
		{
